...
- Paste up to 30 decklists in the **Tourney format** exported from riftbound.gg
...
2. Paste a decklist (Tourney format from riftbound.gg) into each textarea
...
The app only supports the **Tourney format** from riftbound.gg:
...
Cards are scored based on two factors (frequency dominates; intensity breaks ties):

```
frequency = decks where card appears / total valid decks
intensity = total copies across all valid decks / (total valid decks × 3)

score = (frequency × 100) + (intensity × 10)   →  range: 0–110
```
...
- Input is manual only — no riftbound.gg API integration
